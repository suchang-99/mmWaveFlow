...
class SMPL(nn.Module):
    def __init__(self, model_root='model/smpl/pytorch/models/', gender='neutral'):
        ...
        self.shapedirs = torch.tensor(np.array(self.smpl_data['shapedirs']), dtype=torch.float32).cuda()
        self.posedirs = torch.tensor(np.array(self.smpl_data['posedirs']), dtype=torch.float32).cuda()
        self.v_template = torch.tensor(np.expand_dims(self.smpl_data['v_template'], axis=0),
                                       dtype=torch.float32).cuda()
        self.J_regressor = torch.tensor(
            np.array(self.smpl_data['J_regressor'].toarray()), dtype=torch.float32).cuda()
        self.weights = torch.tensor(np.array(self.smpl_data['weights']), dtype=torch.float32).cuda()
        ...
        self.v_template = self.v_template - torch.einsum('vcx,x->vc', self.posedirs, torch.reshape(
            torch.tile(torch.eye(3, dtype=torch.float32).cuda(), [self.num_joints - 1, 1]), [-1]))

    def __call__(self, pose_rotvecs, shape_betas, trans=None, return_vertices=True,return_pelvis=False):
   ...
   """
        ...
        rel_rotmats = rodrigues(pose_rotvecs.view(-1, 3)).view(shape_betas.shape[0], 24, 3, 3)
        # print(rel_rotmats.shape)
        # c=input()
        ...
        if(return_pelvis):
            return vertices, glob_positions, j[:, 0]
        return vertices, glob_positions
