...
class PCGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None, reg_idx=0):
        ...
        for i in range(num_task):
            ...
            proj_indices = list(range(num_task))
            proj_indices.remove(reg_idx)
            random.shuffle(proj_indices)

            for j in proj_indices:
                g_j = grads[j]
                g_i_g_j = torch.dot(g_i, g_j)
                if g_i_g_j < 0:
                    g_i -= (g_i_g_j) * g_j / (g_j.norm() ** 2 + 1e-12)
