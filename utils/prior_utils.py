from __future__ import absolute_import, print_function, print_function
import sys
import os
import time
import pickle
...
class MaxMixturePrior(nn.Module):
    ...
    def __init__(self, prior_folder='prior',
        ...
        # globale_orient=torch.zeros((8,3))
        # pose_param=torch.cat((globale_orient,self.means),dim=1).cuda()
        # _,shape_param,trans_param=uni_utils.get_stand_param(8)
        #
        # d,j=uni_utils.get_smpl_result_no_humanmodel(pose_param,shape_param,trans_param)
        # for i in range(0,8):
        #     visual_utils.compare_mutil_pc([d[i]])
